from transformers import AutoModelForCausalLM, AutoConfig, AutoTokenizer, DataCollatorForLanguageModeling, Trainer, TrainingArguments
from llm_tools_5.utils import create_dataset, tokenize_texts, tokenize_texts_and_labels, get_training_args
...
class MyTrainer:
    ...
    def __init__(self, model_name, x_train, x_test, y_train=None, y_test=None, training_args=None,
        ...
        self.model = AutoModelForCausalLM.from_config(self.model_config, device_map='auto')
        ...
        if y_test is None or y_train is None:
            self.tokenize_func = partial(tokenize_texts, tokenizer=self.tokenizer, context_length=self.context_length)
